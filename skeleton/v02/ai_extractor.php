...
/** OpenAI 호출 공통 */
function openai_chat_json(array $cfg, array $payload): array {
    $apiKey = $cfg['openai_api_key'] ?? '';
    if (!$apiKey) throw new RuntimeException('OpenAI API 키가 설정되지 않았습니다.');
    ...
    $res = curl_exec($ch);
    if ($res === false) throw new RuntimeException('OpenAI 요청 실패: ' . curl_error($ch));
    $status = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);
    if ($status < 200 || $status >= 300) throw new RuntimeException('OpenAI API 에러: HTTP '.$status.' 응답='.$res);
...
function openai_extract_lawsuit(array $cfg, string $subject, string $body): ?array {
    ...
    return openai_chat_json($cfg, $payload);
}
