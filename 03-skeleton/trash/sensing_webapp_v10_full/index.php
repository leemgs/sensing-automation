...
// 사용자가 드롭다운으로 바꿨다면, 바로 active로 저장 (권한 없으면 에러 메시지)
if (isset($_GET['api_id']) && $api_id !== ($apilist['active'] ?? '')) {
    $apilist['active'] = $api_id;
    if (!api_list_save($apilist)) {
        $header_err = "활성 API 저장 실패: llm-api-list.json 쓰기 권한을 확인하세요.";
    }
}
...
if ($action === 'api_manage') {
    ...
    foreach ($items as $it) {
        ...
        <td>
          ...
          <form method='post' action='?action=api_delete' style='display:inline' onsubmit='return confirm(\"정말 삭제하시겠습니까?\")'>
            <input type='hidden' name='api_id' value='{$id}'>
    ...
    <hr>
    <h4>새 항목 추가</h4>
    <form method="post" action="?action=api_new">
    ...
    if ($id && $endpoint && $model && $auth_env) {
        ...
        if ($exists) {
            ...
            if (!api_list_save($apilist)) {
                ...
                $popup = "<div><b>추가 완료</b></div>";
            }
        }
    } else {
        $popup = "<div style='color:#991b1b'><b>오류</b><br>필수 항목이 누락되었습니다.</div>";
    }
}
...
if ($action === 'api_edit') {
    ...
    <form method="post" action="?action=api_save&api_id={$id}">
...
if ($action === 'api_save' && $_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($updated) {
        ...
        $popup = "<div style='color:#991b1b'><b>오류</b><br>대상 ID를 찾을 수 없습니다.</div>";
    }
}
...
if ($action === 'delete_preview' && $_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if (!$nums) {
        ...
        // 서버/계정/비번을 hidden으로 넘겨 접속 실패 방지
        ...
        <form method="post" action="?action=delete" style="margin-top:8px;display:flex;gap:8px;justify-content:flex-end">
          {$hidden}
...
?><!doctype html>
<meta charset="utf-8">
<title>Gmail IMAP → LLM 센싱 자동화 (v10)</title>
<style>
...
<header>
  <div>
    <strong>Gmail IMAP → LLM 센싱 자동화 (v10)</strong>
    <div class="small">
      API: <?=h($active_api['id'] ?? '')?> (모델: <?=h($active_api['model'] ?? '')?> · preset: max_tokens <?=h((string)($active_api['default_max_tokens'] ?? 1400))?> / temp <?=h((string)($active_api['default_temperature'] ?? 0.2))?>)
      &nbsp;|&nbsp; 토큰: <?=$key_status?>
  ...
  <div class="header-right">
    ...
    <a class="btn" href="?action=api_manage">LLM API 관리</a>
    <a class="btn" href="?action=prompt">프롬프트 열람</a>
  </div>
</header>
      ...
      <table>
        ...
        <?php foreach ($emails as $m): ?>
          <tr>
            ...
            <td>
              <a class="btn" href="?action=view&num=<?=$m['num']?>">보기</a>
