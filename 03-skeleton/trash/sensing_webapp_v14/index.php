...
if ($action === 'analyze' && $_SERVER['REQUEST_METHOD']==='POST') {
    ...
    $uidRaw = isset($_POST['uid']) ? (string)$_POST['uid'] : '';
    ...
    if ($server && $email && $pass) {
        $imap = imap_connect($server, $email, $pass);
        if ($imap && $num <= 0 && $uidRaw !== '') { $num = @imap_msgno($imap, (int)$uidRaw); }
        if ($imap) {
        
            if ($num <= 0) { $error = '메일 번호 확인 실패(UID 변환 실패).'; $popup = "<div style='color:#991b1b'><b>오류</b><br>".h($error)."</div>"; imap_close($imap); goto RENDER; }
...
function __showLoading(){
  ...
  if(box){
    ...
    if(span){ span.textContent = '실행중... 0s'; }
  }
  if(__loadTimer) clearInterval(__loadTimer);
  __loadTimer = setInterval(function(){
    var box = document.getElementById('loading').querySelector('.box');
    var secs = Math.floor((Date.now()-__loadStart)/1000);
    if(box){ var span = box.querySelector('span:last-child'); if(span){ span.textContent = '실행중... '+secs+'s'; } }
  }, 1000);
}
function __hideLoading(){
  var el = document.getElementById('loading');
  if(el) el.style.display='none';
  if(__loadTimer) { clearInterval(__loadTimer); __loadTimer=null; }
}
(function(){
