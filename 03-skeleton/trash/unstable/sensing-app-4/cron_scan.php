...
function fail_job(PDO $pdo, string $url, string $err, int $delaySec = 600): void {
    $h = url_hash($url);
...
function retry_failed(PDO $pdo, int $maxRetry = 3): int {
    ...
    foreach ($rows as $r) {
        ...
        try {
            ...
            $pdo->prepare('UPDATE failed_jobs SET attempts = attempts+1, last_error = ?, next_try_at = ? WHERE url_hash = ?')
                ->execute([$e->getMessage(), date('Y-m-d H:i:s', time()+900), url_hash($url)]);
