...
if ($action === 'prompt') {
    ...
    $popup = <<<HTML
    <h4>프롬프트 열람/수정</h4>
    <form method="post" action="?action=save_prompt&server={urlencode($server)}&email={urlencode($email)}&pass={urlencode($pass)}&limit={$limit}&max_tokens={$max_tokens}&temperature={$temperature}">
...
if ($action === 'analyze' && $_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($server && $email && $pass && $num > 0) {
        ...
        if ($imap) {
            ...
            list($ok, $res) = analyze_and_save($plain_for_llm, $max_tokens_p, $temperature_p, $uid, $subject);
            if ($ok) {
...
if ($action === 'delete_preview' && $_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if (!$nums) {
        ...
        // 선택 목록 간단 테이블 생성 (넘겨받은 hidden 데이터에서 subject/from/date도 포함)
        $rows = "";
        ...
        <form method="post" action="?action=delete&server={urlencode($server)}&email={urlencode($email)}&pass={urlencode($pass)}&limit={$limit}&max_tokens={$max_tokens}&temperature={$temperature}" style="margin-top:8px;display:flex;gap:8px;justify-content:flex-end">
          {$hidden}
...
?><!doctype html>
<meta charset="utf-8">
<title>Gmail IMAP → LLM 센싱 자동화 (v08)</title>
<style>
...
<header>
  <div>
    <strong>Gmail IMAP → OpenRouter LLM 센싱 자동화 (v08)</strong>
    <div class="small">IMAP 서버: <?=h($server ?: $env_server)?> &nbsp;|&nbsp; IMAP 이메일: <?=h($email ?: $env_email)?> &nbsp;|&nbsp; OpenRouter 키: <?=$key_status?></div>
  </div>
  <div class="header-right">
    <a class="btn" href="?action=prompt&server=<?=urlencode($server)?>&email=<?=urlencode($email)?>&pass=<?=urlencode($pass)?>&limit=<?=$limit?>&max_tokens=<?=$max_tokens?>&temperature=<?=$temperature?>">프롬프트 열람</a>
...
<main>
  ...
  <?php if ($emails): ?>
    <form method="post" action="?action=delete_preview&server=<?=urlencode($server)?>&email=<?=urlencode($email)?>&pass=<?=urlencode($pass)?>&limit=<?=$limit?>&max_tokens=<?=$max_tokens?>&temperature=<?=$temperature?>" class="card">
      ...
      <table>
        ...
        <?php foreach ($emails as $m): ?>
          <tr>
            ...
            <td>
              <a class="btn" href="?action=view&num=<?=$m['num']?>&server=<?=urlencode($server)?>&email=<?=urlencode($email)?>&pass=<?=urlencode($pass)?>&limit=<?=$limit?>&max_tokens=<?=$max_tokens?>&temperature=<?=$temperature?>">보기</a>
              <form method="post" action="?action=analyze&limit=<?=$limit?>&max_tokens=<?=$max_tokens?>&temperature=<?=$temperature?>" style="display:inline">
                <input type="hidden" name="server" value="<?=h($server)?>">
                <input type="hidden" name="email"  value="<?=h($email)?>">
                <input type="hidden" name="pass"   value="<?=h($pass)?>">
