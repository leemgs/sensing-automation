...
function analyze_and_save(string $emailBody, int $max_tokens, float $temperature, string $uid='', string $subject=''): array {
    ensure_dirs();
    $messages = [
    ...
    $raw = null;
    $payloadUsed = null;
    $ret = llm_chat_complete($messages, $max_tokens, $temperature, $raw);
    $ok = $ret[0]; 
    if (!$ok) return [false, $ret[1]];
    $resp = $ret[1];
    $payloadUsed = $ret[2] ?? null;
