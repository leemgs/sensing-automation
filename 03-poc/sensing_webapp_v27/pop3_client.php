...
function imap_connect(string $server, string $user, string $pass, string $folder='INBOX') {
    // Build POP3 mailbox (Gmail requires 995/SSL)
    $endpoint = "{".$server.":995/pop3/ssl/novalidate-cert}".$folder;

    // Gmail POP quirk: messages retrieved once via POP disappear from future POP listings
    // unless username is prefixed with 'recent:'. We enable this by default for pop.gmail.com,
    // and allow .env overrides: POP3_GMAIL_RECENT=1 (default), POP3_RECENT_MODE=1 (force for any POP).
    $authUser = $user;
    if (function_exists('get_env_value')) {
        $gmailRecent = strtolower((string)(get_env_value('POP3_GMAIL_RECENT') ?? '1'));
        $forceRecent = strtolower((string)(get_env_value('POP3_RECENT_MODE') ?? '0'));
        $truthy = ['1','true','yes','on'];
        if (in_array($forceRecent, $truthy, true)) {
            if (stripos($authUser, 'recent:') !== 0) $authUser = 'recent:'.$authUser;
        } elseif (stripos($server, 'pop.gmail.com') !== false) {
            if (in_array($gmailRecent, $truthy, true) && stripos($authUser, 'recent:') !== 0) {
                $authUser = 'recent:'.$authUser;
            }
        }
    } else {
        if (stripos($server, 'pop.gmail.com') !== false && stripos($authUser, 'recent:') !== 0) {
            $authUser = 'recent:'.$authUser;
        }
    }

    $mbox = @imap_open($endpoint, $authUser, $pass);
    return $mbox ?: null;
}
