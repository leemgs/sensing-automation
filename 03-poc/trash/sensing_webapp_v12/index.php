...
?><!doctype html>
<meta charset="utf-8">
<title>Gmail IMAP → LLM 센싱 자동화 (v12)</title>
<style>
...
<header>
  <div>
    <strong>Gmail IMAP → LLM 센싱 자동화 (v12)</strong>
    <div class="small">
      <!-- API/모델 프리셋 라인 숨김 -->
      토큰: <?=$key_status?>
  ...
  <div class="header-right">
    ...
    <a class="btn" href="?action=api_manage&<?=http_build_query(['server'=>$server,'email'=>$email,'pass'=>$pass,'limit'=>$limit,'page'=>$page,'api_id'=>$api_id])?>">LLM API 관리</a>
      ...
      <table>
        ...
        <?php foreach ($emails_page as $m): ?>
          <tr>
            ...
            <td>
              <a class="btn" href="?<?=http_build_query(['server'=>$server,'email'=>$email,'pass'=>$pass,'limit'=>$limit,'page'=>$page,'api_id'=>$api_id,'action'=>'view','num'=>$m['num']])?>">보기</a>
