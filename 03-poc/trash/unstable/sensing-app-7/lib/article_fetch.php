    ...
    if (!$allowAll && $host) {
        ...
        if ($whitelist) {
            foreach ($whitelist as $dom) {
                if ($host === $dom || substr($host, -strlen('.'.$dom)) === '.'.$dom) { $ok=true; break; }
            }
        }
        if (!$ok && getenv('SENSING_WHITELIST_AUTO')) {
            $whitelist[]=$host;
            file_put_contents($wlFile, json_encode(array_values(array_unique($whitelist)), JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE));
            error_log('[whitelist] added '.$host);
            $ok=true;
        }
        $ok=false;
