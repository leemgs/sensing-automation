...
function check_mailbox(string $label, string $mailbox, string $email, string $pass): void {
    ...
    // Try a lightweight list (first message overview) if exists
    if ($num > 0) {
        $first = 1;
...
// Also print DEFAULT_MAIL_PROTOCOL for reference
...
function check_llm_api(array $api): void {
    ...
    if ($code >= 200 && $code < 300) {
        ...
        // Try to parse error for auth hints
        $j = json_decode($resp, true);
